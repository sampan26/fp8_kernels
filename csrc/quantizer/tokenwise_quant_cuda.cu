...
template<int kThreads_, int dim, typename input_t_, typename output_t_>
struct quantizer_kernel_traits {
    ...
    static_assert(kNBytes_input == 1 || kNBytes_input == 2 || kNBytes_input == 1);
    static constexpr int ThreadElems = kNBytes_input == 4 ? 4 : kNBytes_input == 2 ? 8 : 16;
    static_assert(ThreadElems * kThreads == dim);
};
...
struct Allreduce {
    ...
    template<typename T, typename Operator op>
...
void tokenwise_quantization_kernel(QuantizerParamsBase params) {
    ...
    using output_scales_t = typename Ktraits::output_scales_t;
    ...
    input_t *x = reinterpret_cast<const input_t*>(params.x_ptr) + batch_id * params.x_batch_stride;
